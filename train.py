...
if __name__ == "__main__":
    # 标签的位置
    trainimg_path = '2007_train.txt'
    valimg_path = '2007_val.txt'
    # 获取classes和anchor的位置
    ...
    # 0.1用于验证，0.9用于训练
    #val_split = 0.1
    with open(trainimg_path) as f:
        lines = f.readlines()
    np.random.seed(10101)
    np.random.shuffle(lines)
    np.random.seed(None)

    with open(valimg_path) as f:
    ...
    if True:
        ...
        model.fit(data_generator(lines, batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
                 steps_per_epoch=max(1, num_train//batch_size),
                validation_data=data_generator(val_lines, batch_size, input_shape, anchors, num_classes, mosaic=False),
        ...
        model.save_weights('./model_data/myyolov4_1.h5')
    ...
    if True:
        ...
        print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
        model.fit_generator(data_generator(lines, batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
                steps_per_epoch=max(1, num_train//batch_size),
                validation_data=data_generator(val_lines, batch_size, input_shape, anchors, num_classes, mosaic=False),
                validation_steps=max(1, num_val//batch_size),
                epochs=Epoch,
                ...
                max_queue_size=1,
                callbacks=[logging, checkpoint, reduce_lr, early_stopping])
        model.save_weights('./model_data/myyolov4_2.h5')
